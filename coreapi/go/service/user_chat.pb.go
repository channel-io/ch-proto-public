...
const file_coreapi_service_user_chat_proto_rawDesc = "" +
	...
	"\rchat_sessions\x18\x01 \x03(\v2\x1a.coreapi.model.ChatSessionR\fchatSessions\"\xce\x05\n" +
	...
	"\x12original_time_zone\x18\b \x01(\tR\x10originalTimeZone:\x88\x02\xbaH\x84\x02\x1a\x81\x02\n" +
	"1search_user_chat_messages.range_less_than_30_days\x127the range between from and to must be less than 30 days\x1a\x92\x01(has(this.from) == has(this.to)) && (!has(this.from) || ((this.to - this.from >= duration('0s')) && (this.to - this.from < duration('2592000s'))))\"\x98\x02\n" +
